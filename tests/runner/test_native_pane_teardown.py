...
async def test_a_turn_bound_mid_scan_makes_the_reaper_rearm_instead_of_reap(
    ...
    # Relative to now, not 0.0: time.monotonic() counts from boot, so on a
    # freshly booted host 0.0 is less than one idle window ago.
    idle_since = time.monotonic() - 10 * 3600
    rig.reaper._last_busy_at[rig.conv_id] = idle_since
    ...
    try:
        ...
        assert rig.alive()
        rearmed = rig.reaper._last_busy_at[rig.conv_id]
        assert rearmed > idle_since
        assert rearmed >= scan_started
    finally:
        rig.app.state.active_turns.pop(rig.conv_id, None)
